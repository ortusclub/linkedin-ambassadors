    ...
    {open && (
      <div style={{ borderTop: "1px solid var(--border,#eee)", background: "var(--panel,#fafafa)", padding: "16px 16px 18px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 16 }}>
          ...
          <D label="Ambassador payout">{r.ambassadorPayment ? `${formatMoney(r.ambassadorPayment, currencyConfigFor(r.payoutCurrency, r.referredBy).currency)}/mo` : null}</D>
...
export default function OnboardingPage() {
  ...
  const bucketed = useMemo(() => {
    ...
    // Initial reads newest-applied first — the freshest applications sit on top.
    // Onboarded reads newest-first too, most recent wins on top.
