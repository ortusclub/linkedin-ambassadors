...
// Owner-side photo upload during self-service intake, gated by the referrer's public slug.
export async function POST(req: Request, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (!me || !me.active) return NextResponse.json({ error: "This onboarding link is not available." }, { status: 404 });
    ...
    console.error("Owner photo upload failed", error instanceof Error ? error.name : "unknown");
