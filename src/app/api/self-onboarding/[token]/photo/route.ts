...
import { resolveSelfSession } from "@/lib/self-service-onboarding";
...
// DIY mirror: token-gated profile-photo upload during self-service onboarding.
export async function POST(req: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    await resolveSelfSession(token); // authorize (throws 404 if the token isn't valid)
    ...
    console.error("DIY onboarding photo upload failed", error instanceof Error ? error.name : "unknown");
