  ...
  return <>
    ...
    {!setup.configured ? <div className={styles.note}>Email setup is not switched on yet. Your progress is saved. The team needs to finish setting up the email domains before you can add an address to LinkedIn.</div> : <>
      {!setup.forwardingActive && <form onSubmit={e => { e.preventDefault(); void submit({ action: "start", destination, consent }); }}>
        ...
        <label className={styles.field}>The account owner&apos;s email address<input type="email" required maxLength={254} value={destination} disabled={setup.destinationVerified} onChange={e => setDestination(e.target.value)} placeholder="owner@example.com" /></label>
        <p className={styles.hint}>Enter an email the owner can open right now. We&apos;ll send a 6-digit code here to confirm it works. During setup, the account&apos;s LinkedIn codes are also forwarded here for about an hour so the owner can read them.</p>
